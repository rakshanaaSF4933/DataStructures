...
namespace ECommerceList
{
    ...
    public static class Shopping
    {
        ...
        public static void CustomerLogin()
        {
            ...
            foreach (CustomerDetails data in customers)
            {
                list.Add(data.CustomerID);
            }
            found = search.Search(list, customerID);
            if (found == -1)
            {
        ...
        public static void Purchase()
        {
            ...
            BinarySearch<string> search = new BinarySearch<string>();
            CustomList<string> list = new CustomList<string>();
            foreach (ProductDetails data in products)
            {
                list.Add(data.ProductID);
            }
            found = search.Search(list, productID);
            if (found == -1)
            {
            ...
            {
                ProductDetails product = products[found];
        ...
        public static void CancelOrder()
        {
            ...
            if (noOrder == 1)
            {
                ...
                int found;
                BinarySearch<string> search = new BinarySearch<string>();
                CustomList<string> customerIDList = new CustomList<string>();
                CustomList<string> orderIDList = new CustomList<string>();
                foreach (OrderDetails data in orders)
                {
                    orderIDList.Add(data.OrderID);
                    if (customer.CustomerID.Equals(data.CustomerID))
                    {
                        customerIDList.Add(data.OrderID);
                    }
                }
                ...
                {
                    found = search.Search(customerIDList, orderID);
                    if (found == -1)
                    {
                    ...
                    {
                        //FInd customer order in orders list
                        int orderFound = search.Search(orderIDList, orderID);
                        System.Console.WriteLine(orderFound);
